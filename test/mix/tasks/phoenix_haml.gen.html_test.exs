...
defmodule PhoenixHaml.DupHTMLController do
end
...
defmodule Mix.Tasks.PhoenixHaml.Gen.HtmlTest do
  ...
  test "generates html resource" do
    in_tmp "generates html resource", fn ->
      ...
      assert_file "web/controllers/user_controller.ex", fn file ->
        assert file =~ "defmodule PhoenixHaml.UserController"
        assert file =~ "use PhoenixHaml.Web, :controller"
        assert file =~ "Repo.get!"
      end

      assert_file "web/views/user_view.ex", fn file ->
        assert file =~ "defmodule PhoenixHaml.UserView do"
        assert file =~ "use PhoenixHaml.Web, :view"
      end

      assert_file "web/templates/user/edit.html.haml", fn file ->
        assert file =~ "action: user_path(@conn, :update, @user)"
      end

      assert_file "web/templates/user/form.html.haml", fn file ->
      ...
      assert_file "web/templates/user/new.html.haml", fn file ->
        assert file =~ "action: user_path(@conn, :create)"
      end

      assert_file "web/templates/user/show.html.haml", fn file ->
        assert file =~ "%strong Name:"
        assert file =~ "= @user.name"
      end

      assert_file "test/controllers/user_controller_test.exs", fn file ->
        assert file =~ "defmodule PhoenixHaml.UserControllerTest"
        assert file =~ "use PhoenixHaml.ConnCase"
  ...
  test "generates nested resource" do
    in_tmp "generates nested resource", fn ->
      ...
      assert_file "web/controllers/admin/super_user_controller.ex", fn file ->
        assert file =~ "defmodule PhoenixHaml.Admin.SuperUserController"
        assert file =~ "use PhoenixHaml.Web, :controller"
        assert file =~ "Repo.get!"
      end

      assert_file "web/views/admin/super_user_view.ex", fn file ->
        assert file =~ "defmodule PhoenixHaml.Admin.SuperUserView do"
        assert file =~ "use PhoenixHaml.Web, :view"
      end

      assert_file "web/templates/admin/super_user/edit.html.haml", fn file ->
        assert file =~ "%h2 Edit super user"
        assert file =~ "action: super_user_path(@conn, :update, @super_user)"
      end

      assert_file "web/templates/admin/super_user/form.html.haml", fn file ->
        assert file =~ ~s(= text_input f, :name, class: "form-control")
      end
      ...
      assert_file "web/templates/admin/super_user/show.html.haml", fn file ->
  ...
  test "generates html resource without model" do
    in_tmp "generates html resource without model", fn ->
      ...
      assert_file "web/templates/admin/user/form.html.haml", fn file ->
